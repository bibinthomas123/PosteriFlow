...
class MetricsComputer:
    ...
    @staticmethod
    def snr_regime(snr):
        """Classify SNR into regimes using configured SNR_RANGES."""
        try:
...
# ============================================================================
# DATASET LOADING (STREAMING - MEMORY EFFICIENT)
# ============================================================================


def iter_samples_from_split(split_dir, max_samples=None):
    """
    Stream samples from chunks in a split directory (memory efficient).
    
    Args:
        split_dir: Path to split directory (train/validation/test)
        max_samples: Max samples to yield (None = all)
    
    Yields:
        dict: Individual sample from chunks
    """
    ...
    for chunk_id, chunk_file in enumerate(chunk_files):
        if not chunk_file.exists():
            WARN(f"Chunk file not found: {chunk_file}")
            continue
        ...
        try:
            ...
            for sample in chunk_data:
                ...
                if max_samples and total_yielded >= max_samples:
                    INFO(f"Reached max samples limit: {max_samples}")
                    return
            ...
            WARN(f"Failed to load chunk {chunk_file.name}: {e}")
            continue


def load_dataset(data_dir, max_samples=None):
    """
    Load all samples from train/validation/test splits using streaming.
    
    Streams chunks one at a time (memory efficient) instead of loading all at once.
    ...
    Returns:
        list: All samples from all splits
    """
    all_samples = []
    ...
    for split in ["train", "validation", "test"]:
        ...
        if not split_dir.exists():
            INFO(f"Split {split}/ not found, skipping")
            continue

        chunk_files = sorted(split_dir.glob("chunk_*.pkl"))
        if not chunk_files:
            INFO(f"No chunks found in {split}/")
            continue
        
        INFO(f"Loading {len(chunk_files)} chunk(s) from {split}/ (streaming)...")

        split_count = 0
        for sample in iter_samples_from_split(split_dir, max_samples=max_samples):
            if sample is not None:
                ...
                if total_loaded % 1000 == 0:
                    INFO(f"Loaded {total_loaded} samples total")

                if max_samples and total_loaded >= max_samples:
                    INFO(f"Reached global max samples: {max_samples}")
                    return all_samples

        INFO(f"Streamed {split_count} samples from {split}/")

    INFO(f"Total: {len(all_samples)} samples loaded (streaming)")
    return all_samples
...
def extract_parameters(samples):
    ...
    for idx, sample in enumerate(samples):
        ...
        row = {
            ...
            "mass_ratio": (
                params.get("mass_2", 0) / params.get("mass_1", 1)
                if params.get("mass_1", 0) > 0
                else np.nan
            ),
            ...
            "a1": params.get("a1") or params.get("a_1"),
            "a2": params.get("a2") or params.get("a_2"),
            "tilt1": params.get("tilt1") or params.get("tilt_1"),
            "tilt2": params.get("tilt2") or params.get("tilt_2"),
            "phi_12": params.get("phi_12"),
            "phi_jl": params.get("phi_jl"),
            "chi_eff": params.get("chi_eff") or params.get("effective_spin"),
    ...
    # ✅ ADD SNR REGIME COLUMN - USE CONFIG VALUES, NOT HARDCODED
    if "network_snr" in df.columns:
        # Use SNR_RANGES from config for consistency with dataset generation
        try:
            from ahsd.data.config import SNR_RANGES
        except ImportError:
            # Fallback if config not available
        ...
        def classify_snr(snr):
            """Classify SNR using configured ranges (matches dataset_generator.py)"""
            if pd.isna(snr):
                return "unknown"
        ...
        INFO(f"Added SNR regime classification using configured SNR_RANGES")
...
def analyze_correlations(df, output_dir):
    ...
    for evt in ["BBH", "BNS", "NSBH"]:
        ...
        if mask.sum() > 10:
            ...
            rho = safe_correlation(
                df[mask]["luminosity_distance"], df[mask]["target_snr"], "spearman"
            )
            tau = safe_correlation(
                df[mask]["luminosity_distance"], df[mask]["target_snr"], "kendall"
            )
...
def analyze_snr_regimes(df, output_dir):
    ...
    # Define SNR regimes from config (not hardcoded)
    # Import SNR_RANGES from config
    try:
        from ahsd.data.config import SNR_RANGES

        regimes = {
            "weak": SNR_RANGES.get("weak", (10.0, 15.0)),
            "low": SNR_RANGES.get("low", (15.0, 25.0)),
            "medium": SNR_RANGES.get("medium", (25.0, 40.0)),
            "high": SNR_RANGES.get("high", (40.0, 60.0)),
            "loud": SNR_RANGES.get("loud", (60.0, np.inf)),
        }
    except ImportError:
        # Fallback to defaults if config can't be imported
        regimes = {
            "weak": (10.0, 15.0),
            "low": (15.0, 25.0),
            "medium": (25.0, 40.0),
    ...
    for regime_name, (low, high) in regimes.items():
        ...
        if count > 0:
            ...
            INFO(
                f"   {regime_name.upper():8s} ({low:>3.0f}-{high:>3.0f}): "
...
def check_physics_correctness(df, violations):
    ...
    for event_type in ["BBH", "BNS", "NSBH"]:
        ...
        if mask.sum() > 10:
            ...
            if mask_non_edge.sum() > 10:
                r = safe_correlation(
                    df[mask_non_edge]["luminosity_distance"], df[mask_non_edge]["target_snr"]
                )
                ...
                INFO(
                    f"   {status} {event_type}: r={r:.3f} ({mask_non_edge.sum()} non-edge samples)"
                )

            # Also show overall for context
            r_all = safe_correlation(df[mask]["luminosity_distance"], df[mask]["target_snr"])
    ...
    for event_type in ["BBH", "BNS", "NSBH"]:
        mask = df["event_type"] == event_type
        if mask.sum() > 10:
            r = safe_correlation(df[mask]["total_mass"], df[mask]["luminosity_distance"])
    ...
    if "redshift" in df.columns and "luminosity_distance" in df.columns:
        ...
        if total_cosmo > 0:
            ...
            INFO(
                f"   Valid: {valid_cosmology}/{total_cosmo} ({100*valid_cosmology/total_cosmo:.1f}%)"
            )
            if invalid_cosmology > 0:
                INFO(
                    f"   Invalid: {invalid_cosmology} ({100*invalid_cosmology/max(total_cosmo,1):.1f}%)"
                )
    else:
...
def check_overlap_quality(df):
    ...
    INFO(
        f"   SNR range: {overlap_df['network_snr'].min():.1f} - {overlap_df['network_snr'].max():.1f}"
    )
    INFO(
        f"   SNR mean: {overlap_df['network_snr'].mean():.1f} ± {overlap_df['network_snr'].std():.1f}"
    )
...
def plot_research_figure_1_composition(output_dir):
    ...
    ax.text(
        5,
        9.5,
        "Gravitational Wave Dataset Composition",
        fontsize=18,
        fontweight="bold",
        ha="center",
    )
    ax.text(
        5,
        9.0,
        "Signal generation → Noise injection → Overlap builder → Feature extraction",
        fontsize=11,
        ha="center",
        style="italic",
        color="gray",
    ...
    for x, y, text, color in stats:
        ax.text(
            ...
            fontweight="bold",
            bbox=dict(
                boxstyle="round", facecolor=color, alpha=0.7, edgecolor="black", linewidth=1.5
            ),
        )
...
def plot_research_figure_2_signals(df, output_dir):
    ...
    # Overlapping signals
    signal3 = np.sin(2 * np.pi * 50 * t) * np.exp(-t / 0.2) + 0.5 * np.sin(
        2 * np.pi * 80 * (t - 0.1)
    ) * np.exp(-(t - 0.1) / 0.15)
...
def plot_research_figure_3_mass(df, output_dir):
    ...
    if len(m1) > 10 and len(m2) > 10:
        ...
        # Event type regions
        ax.axhspan(
            1, 3, alpha=0.15, color="blue", linewidth=2, edgecolor="blue", label="BNS region"
        )
        ax.axhspan(
            3, 100, alpha=0.15, color="red", linewidth=2, edgecolor="red", label="BBH region"
        )
...
def plot_research_figure_5_distance_snr(df, output_dir):
    ...
    if "snr_regime" not in df.columns:
        ...
        def classify_snr(snr):
            """Classify SNR using configured ranges"""
            if pd.isna(snr):
                return "unknown"
    ...
    for idx, evt in enumerate(event_types):
        ...
        if len(subset) > 0:
            for regime in ["weak", "low", "medium", "high", "loud"]:
                ...
                if regime_mask.sum() > 0:
                    ax.scatter(
                        subset[regime_mask]["luminosity_distance"],
        ...
        if len(subset_medium) > 0:
            ax.scatter(
                subset_medium["luminosity_distance"],
            ...
            r = safe_correlation(subset_medium["luminosity_distance"], subset_medium["network_snr"])
...
def plot_research_figure_9_physics(df, output_dir):
    ...
    theta = (
        ...
        if "inclination" in df.columns
        else df["theta_jn"].dropna() if "theta_jn" in df.columns else pd.Series()
    )
    if len(theta) > 0:
...
def generate_comprehensive_report(
    df, violations, all_event_types, corr_results, regime_stats, output_dir
):
...
def check_noise_quality(samples, df, output_dir):
    """
    Memory-efficient comprehensive noise quality validation.
    Uses streaming/batched processing to handle large datasets (50k+ samples).

    Checks:
    1. Presence of noise data in samples
    2. Noise statistics (mean, std, power) - computed on batches
    ...
    """
    INFO("\n" + "=" * 80)
    INFO("🔊 NOISE QUALITY VALIDATION (Memory-Efficient Mode)")
    INFO("=" * 80)
    ...
    BATCH_SIZE = 500  # Process samples in batches
    SAMPLE_FOR_STATS = 1000  # Sample 1000 points per sample for stats

    # 1. Check presence of noise data (single pass)
    INFO("\n1️⃣  Noise Data Presence:")
    samples_with_noise = 0
    samples_without_noise = 0
    first_valid_noise = None  # For PSD analysis later
    noise_stds = []  # For stationarity check
    nan_count = 0
    inf_count = 0
    total_points = 0
    dead_samples = 0
    ...
    for idx, sample in enumerate(samples):
        ...
        # Extract noise efficiently
        noise = (
            sample.get("noise")
            or sample.get("psd_noise")
            or sample.get("strain_noise")
            or sample.get("background_noise")
        )
                ...
                for det_name, det_info in det_data.items():
                    if isinstance(det_info, dict) and "noise" in det_info:
                        ...
                        if det_noise is not None and isinstance(det_noise, (np.ndarray, list)):
                            det_noise = np.asarray(det_noise, dtype=np.float32)
                            if len(det_noise) > 0:
        ...
        if noise is not None and isinstance(noise, (np.ndarray, list)):
            ...
            if len(noise) > 0:
                ...
                # Store first valid noise for PSD
                if first_valid_noise is None:
                    first_valid_noise = noise.copy()

                # Streaming stats: std and integrity checks (on subsample)
                noise_finite = noise[np.isfinite(noise)]
                if len(noise_finite) > 0:
                    noise_stds.append(np.std(noise_finite))
                    nan_count += np.isnan(noise).sum()
                    inf_count += np.isinf(noise).sum()
    ...
    INFO(
        f"   {status} Samples with noise: {samples_with_noise}/{total_samples} ({pct_with_noise:.1f}%)"
    )
    ...
    # 2. Noise statistics (batch streaming)
    if samples_with_noise > 0:
        INFO("\n2️⃣  Noise Statistics (Streaming):")

        # Use Welford's algorithm for numerically stable online mean/std
        n = 0
        mean = 0.0
        M2 = 0.0  # For variance
        min_val = np.inf
        ...
        # Stream through samples in batches
        for idx, sample in enumerate(samples):
            if sample is None:
                continue
            ...
            if noise is not None and isinstance(noise, (np.ndarray, list)):
                noise = np.asarray(noise, dtype=np.float32)
                if len(noise) > 0:
                    # Subsample if too large to avoid memory spike
                    if len(noise) > SAMPLE_FOR_STATS:
                        noise_subset = noise[:: len(noise) // SAMPLE_FOR_STATS][:SAMPLE_FOR_STATS]
                    else:
                        noise_subset = noise

                    noise_finite = noise_subset[np.isfinite(noise_subset)]

                    # Welford update
                    for val in noise_finite:
                        n += 1
                        delta = val - mean
                        mean += delta / n
                        delta2 = val - mean
            ...
            if (idx + 1) % 5000 == 0:
                INFO(f"   Processed {idx + 1}/{total_samples} samples...")

        if n > 0:
            variance = M2 / n
            noise_std = np.sqrt(variance)
            noise_rms = np.sqrt(sum_sq / n)
            ...
            INFO(f"   Range: [{min_val:.2e}, {max_val:.2e}]")

            # Check noise is centered at zero
            rms_to_std_ratio = noise_rms / max(noise_std, 1e-30)
            if rms_to_std_ratio < 0.5:
                WARN(f"   RMS/std ratio ({rms_to_std_ratio:.3f}) unusually low!")
                noise_metrics["issues"].append("Noise centering issue")
            else:
                INFO(
                    f"   ✓ Noise properly centered at zero (RMS/std ratio: {rms_to_std_ratio:.3f})"
                )
    ...
    # 3. PSD validation (on first valid sample only)
    INFO("\n3️⃣  PSD Validation:")

    if first_valid_noise is not None and len(df) > 0:
        if len(first_valid_noise) >= 128:
            try:
                # Use a segment for FFT to save memory
                segment_len = min(len(first_valid_noise), 8192)
                sample_noise = first_valid_noise[:segment_len].astype(np.float32)

                # Compute FFT
                fft_vals = np.fft.fft(sample_noise)
                freqs = np.fft.fftfreq(len(sample_noise), d=1.0 / 4096)

                # PSD
                psd = np.abs(fft_vals) ** 2 / segment_len
                ...
                if freq_mask.sum() > 0:
                    ...
                    nonzero_psd = psd_range[psd_range > 1e-50]
                    if len(nonzero_psd) > 10:
                        log_psd = np.log10(nonzero_psd)
                        log_std = np.std(log_psd)

                        if log_std < 0.15:
                            WARN(f"   PSD unnaturally flat (log_std = {log_std:.4f})")
                            noise_metrics["issues"].append("PSD too uniform")
                        else:
                            INFO(
                                f"   ✓ PSD shows realistic frequency dependence (log_std={log_std:.4f})"
                            )

                # Clean up
                del fft_vals, psd, freqs
    ...
    if samples_with_noise > 0 and len(df) > 0:
        # Estimate noise power from first sample (representative)
        if first_valid_noise is not None:
            noise_power_avg = np.mean(first_valid_noise**2)
            ...
            if len(snr_values) > 0:
                ...
                INFO(f"   Average noise power (sample): {noise_power_avg:.2e}")
    ...
    if len(noise_stds) > 10:
        ...
        cv = std_std / std_mean if std_mean > 0 else 0
        ...
        if cv > 0.4:
            ...
            if cv == 0:
                INFO(f"   ✓ Synthetic noise - uniform statistics expected (CV=0)")
            else:
                INFO(f"   ✓ Noise statistics show expected variability (CV={cv:.3f})")

    # 6. Data integrity (already computed in pass 1)
    INFO("\n6️⃣  Data Integrity Checks:")

    if total_points > 0:
        nan_pct = 100 * nan_count / total_points
        inf_pct = 100 * inf_count / total_points
    ...
    # Check for dead channels (one more pass, but only counts)
    ...
    for idx, sample in enumerate(samples):
        ...
        if (idx + 1) % 5000 == 0:
            INFO(f"   Dead channel check: {idx + 1}/{total_samples}...")
...
def plot_research_figure_16_overlap_heatmap(df, output_dir):
    ...
    overlap_df["signal_scenario"] = pd.cut(
        overlap_df["num_signals"],
        bins=[1, 2, 3, 4, 100],
    ...
    contingency = pd.crosstab(
        overlap_df["signal_scenario"], overlap_df["snr_regime"], margins=False
    )
    ...
    ax.text(
        ...
        verticalalignment="top",
        bbox=dict(
            boxstyle="round", facecolor="lightyellow", alpha=0.9, edgecolor="black", linewidth=1.5
        ),
        family="monospace",
    )
...
def plot_research_figure_17_spin_tilt_physics(df, output_dir):
    ...
    # a1 vs tilt1
    spin_cols_a = [("a1", "tilt1"), ("spin1", "tilt1"), ("a_1", "tilt1")]
    a_cols = None
    for a_col, tilt_col in spin_cols_a:
        if a_col in df.columns and tilt_col in df.columns:
            a_cols = (a_col, tilt_col)
            break

    if a_cols:
        mask = df[[a_cols[0], a_cols[1]]].notna().all(axis=1)
        if mask.sum() > 10:
            ax = axes[0, 0]
            x, y = df[mask][a_cols[0]], df[mask][a_cols[1]]
            scatter = ax.scatter(
                x,
                y,
                alpha=0.5,
                s=40,
                c=df[mask]["network_snr"],
                cmap="viridis",
                edgecolors="black",
                linewidth=0.5,
            )
    ...
    # a2 vs tilt2
    spin_cols_b = [("a2", "tilt2"), ("spin2", "tilt2"), ("a_2", "tilt2")]
    b_cols = None
    for a_col, tilt_col in spin_cols_b:
        if a_col in df.columns and tilt_col in df.columns:
            b_cols = (a_col, tilt_col)
            break

    if b_cols:
        mask = df[[b_cols[0], b_cols[1]]].notna().all(axis=1)
        if mask.sum() > 10:
            ax = axes[0, 1]
            x, y = df[mask][b_cols[0]], df[mask][b_cols[1]]
            scatter = ax.scatter(
                x,
                y,
                alpha=0.5,
    ...
    inclination_col = (
        "inclination"
        if "inclination" in df.columns
        else "theta_jn" if "theta_jn" in df.columns else None
    )
    if inclination_col:
...
def plot_research_figure_19_snr_efficiency(df, output_dir):
    ...
    labels = [
        et
        for et, d in zip(
            event_types, [df[df["event_type"] == et]["network_snr"].dropna() for et in event_types]
        )
        if len(d) > 0
    ]
    ...
    if len(regime_data_filtered) > 0:
        try:
            parts = ax.violinplot(
                regime_data_filtered, positions=regime_positions, showmeans=True, showmedians=True
            )
...
def main():
    ...
    generate_comprehensive_report(
        df, violations, all_event_types, corr_results, regime_stats, args.output_dir
    )
