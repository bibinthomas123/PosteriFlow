...
class ParameterSampler:
    ...
    def sample_bbh_parameters(self, snr_regime: Optional[str] = None, is_edge_case: bool = False) -> Dict:
        """
        BBH sampler with strict mass–distance decoupling:
        - Sample source-frame masses from astrophysical prior (independent of distance)
        - Sample redshift from comoving-volume prior (independent of masses)
        - Map z -> d_L via cosmology (NO CLIPPING). If d_L outside allowed range, resample z only.
        - Detector-frame masses: m_det = m_src * (1 + z)
        - Target SNR sampled independent of mass/distance
        """

        # -----------------------------
        # 1) Priors & ranges (source-frame)
        # -----------------------------
        # Prefer config; fall back to conservative defaults that reduce high-z dominance
        mmin, mmax = (self.mass_ranges.get("BBH", (5.0, 60.0)))
        dmin, dmax = (self.distance_ranges.get("BBH", (50.0, 10000.0)))  # wide enough to avoid boundary pileup
        # Reasonable horizon for BBH at design sensitivity; use z upper ~ 2
        z_min, z_max = 0.001, 2.0

        # -----------------------------
        # 2) Sample intrinsic (source-frame) masses (independent of distance)
        #    Use a power-law and enforce ordering m1>=m2
        # -----------------------------
        m1_src = float(_sample_powerlaw(mmin, mmax, alpha=2.3))
        m2_src = float(_sample_powerlaw(mmin, mmax, alpha=2.3))
        # tiny jitter to avoid exact ties in downstream sorting
        m1_src += float(np.random.uniform(-0.01, 0.01))
        m2_src += float(np.random.uniform(-0.01, 0.01))
        mass_1_src, mass_2_src = (m1_src, m2_src) if m1_src >= m2_src else (m2_src, m1_src)

        # -----------------------------
        # 3) Sample redshift from comoving volume prior, then compute d_L
        #    IMPORTANT: never clip d_L. If outside [dmin, dmax], resample z only.
        # -----------------------------
        for _ in range(128):  # hard cap to avoid pathological loops
            z = sample_redshift_from_comoving_volume(z_min=z_min, z_max=z_max)
            d_L = compute_luminosity_distance_from_redshift(z)
            if dmin <= d_L <= dmax:
                break
        else:
            # Fallback: if we failed repeatedly, just pin to the closest bound but DO NOT alter z<->d_L mapping again.
            # (Extremely rare with sensible ranges.)
            if d_L < dmin:
                d_L = float(dmin)
                z   = float(calculate_redshift(d_L))
            elif d_L > dmax:
                d_L = float(dmax)
                z   = float(calculate_redshift(d_L))

        # Comoving distance for bookkeeping/plots
        d_C = float(calculate_comoving_distance(z)) if z else float(d_L)

        # -----------------------------
        # 4) Detector-frame masses
        # -----------------------------
        mass_1 = float(mass_1_src * (1.0 + z))
        mass_2 = float(mass_2_src * (1.0 + z))
        total_mass = mass_1 + mass_2
        chirp_mass = (mass_1 * mass_2) ** (3.0 / 5.0) / (total_mass ** (1.0 / 5.0))
        chirp_mass_src = (mass_1_src * mass_2_src) ** (3.0 / 5.0) / ((mass_1_src + mass_2_src) ** (1.0 / 5.0))

        # -----------------------------
        # 5) Target SNR (independent of mass/distance)
        # -----------------------------
        target_snr = self._sample_target_snr(snr_regime) if snr_regime else self._sample_target_snr_from_distribution()

        # -----------------------------
        # 6) Spins & geometry (isotropic)
        # -----------------------------
        a1  = float(np.random.beta(2, 5))
        a2  = float(np.random.beta(2, 5))
        tilt1 = float(np.arccos(np.random.uniform(-1.0, 1.0)))
        tilt2 = float(np.arccos(np.random.uniform(-1.0, 1.0)))
        ra    = float(np.random.uniform(0.0, 2.0 * np.pi))
        dec   = float(np.arcsin(np.random.uniform(-1.0, 1.0)))
        theta_jn = float(np.arccos(np.random.uniform(-1.0, 1.0)))
        psi   = float(np.random.uniform(0.0, np.pi))
        phase = float(np.random.uniform(0.0, 2.0 * np.pi))
        geocent_time = float(np.random.uniform(-0.1, 0.1))

        # -----------------------------
        # 7) Optional edge cases: resample *source-frame* masses only.
        #    Keep z/d_L unchanged to preserve independence.
        # -----------------------------
        edge_case_type = None
        if is_edge_case:
            if np.random.rand() < 0.5:
                edge_case_type = "extreme_mass_ratio"
                mass_1_src = float(np.random.uniform(30.0, 80.0))
                mass_2_src = float(np.random.uniform(5.0, 30.0))
            else:
                edge_case_type = "short_bbh"
                mass_1_src = float(np.random.uniform(60.0, 100.0))
                mass_2_src = float(np.random.uniform(60.0, 100.0))

            if mass_1_src < mass_2_src:
                mass_1_src, mass_2_src = mass_2_src, mass_1_src

            # Transform to detector frame with SAME z
            mass_1 = float(mass_1_src * (1.0 + z))
            mass_2 = float(mass_2_src * (1.0 + z))
            total_mass   = mass_1 + mass_2
            chirp_mass   = (mass_1 * mass_2) ** (3.0 / 5.0) / (total_mass ** (1.0 / 5.0))
        ...
        params = {
            ...
            "phi12": float(np.random.uniform(0.0, 2.0 * np.pi)),
            "phi_jl": float(np.random.uniform(0.0, 2.0 * np.pi)),
            "theta_jn": theta_jn, "ra": ra, "dec": dec, "psi": psi, "phase": phase,
            ...
            # SNR targets (kept independent of mass/distance)
    ...
    def sample_bns_parameters(self, snr_regime: Optional[str] = None, is_edge_case: bool = False) -> Dict:
        ...
        # Use z_max that gives distances up to the maximum range
        d_max = self.distance_ranges.get("BNS", (10.0, 1200.0))[1]
        z_max_bns = 0.2  # z corresponding to ~1200 Mpc
        z = sample_redshift_from_comoving_volume(z_min=0.001, z_max=z_max_bns)
    ...
    def sample_nsbh_parameters(self, snr_regime: Optional[str] = None, is_edge_case: bool = False) -> Dict:
        ...
        # Sample redshift independently from comoving volume prior
        # Use z_max that gives distances up to the maximum range
        d_max = self.distance_ranges.get("NSBH", (20.0, 1600.0))[1]
        z_max_nsbh = 0.25  # z corresponding to ~1600 Mpc
        z = sample_redshift_from_comoving_volume(z_min=0.001, z_max=z_max_nsbh)
