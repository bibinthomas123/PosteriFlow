#!/usr/bin/env python3
"""Phase 3A: Optimized Neural PE Training Script"""
...
def train_neural_pe(neural_pe: NeuralPENetwork, dataset: AdaptiveSubtractorDataset, epochs: int = 40) -> Dict[str, Any]:
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (waveforms, true_params, qualities) in enumerate(pbar):
            ...
            if batch_idx % 10 == 0:
                ...
                pbar.set_postfix({
                    'loss': f'{loss.item():.4f}',
                    'acc': f'{np.mean(epoch_accuracies[-10:]):.3f}' if epoch_accuracies else '0.000',
                    'lr': f'{current_lr:.1e}'
                })
        ...
        if epoch % 5 == 0:
            current_lr = optimizer.param_groups[0]['lr']
            logging.info(f"Neural PE Epoch {epoch}: Loss = {avg_loss:.6f}, Accuracy = {avg_accuracy:.3f}, Best = {best_accuracy:.3f}, LR = {current_lr:.2e}")
    ...
    print(f"✅ Training Completed: {len(training_history['losses'])} epochs")
    ...
    if debug_samples:
        ...
        pred_magnitude_check = np.abs(sample['pred_params']).max()
        true_magnitude_check = np.abs(sample['true_params']).max()
        scale_ratio = pred_magnitude_check / true_magnitude_check if true_magnitude_check > 0 else 0
        
        if pred_magnitude_check < 0.01:
            print("⚠️  WARNING: Predictions are near zero!")
        elif pred_magnitude_check > 10:
            print("⚠️  WARNING: Predictions are extreme!")
        elif scale_ratio < 0.1:
            print("⚠️  WARNING: Predictions are too small compared to true values")
        elif scale_ratio > 2.0:
            print("⚠️  WARNING: Predictions are too large compared to true values")
        else:
...
def main():
    ...
    final_acc = pe_results['best_accuracy']
    if final_acc >= 0.90:
        print("\n🏆 EXCELLENT: Ready for high-performance Phase 3B!")
    elif final_acc >= 0.85:
        print("\n✅ VERY GOOD: Ready for Phase 3B!")
    elif final_acc >= 0.80:
