...
def sample_overlap_size(p_heavy=0.45, base_lambda=2.2, heavy_min=4, heavy_max=6, max_overall = None):
    """
    Mixture sampler:
...
class GWDatasetGenerator:
    ...
    def __init__(
        ...
        self.max_overlapping_signals = self.config.get("max_overlapping_signals", 10)
    ...
    def _get_noise_for_detector(self, detector_name: str, psd_dict: Dict) -> Tuple[np.ndarray, str]:
        ...
        if self.use_real_noise_prob > 0 and np.random.random() < self.use_real_noise_prob:
            ...
            if (
                ...
                try:
                    ...
                    # CRITICAL: Check for NaN/Inf before normalizing (mirrors cached segment behavior)
                    if not np.all(np.isfinite(noise)):
                        self.logger.debug(f"Real noise for {detector_name} contains NaN/Inf, falling back to synthetic")
                        raise ValueError("Real noise has NaN/Inf values")
        ...
        # CRITICAL: Check for NaN/Inf before normalizing (robustness against generator failures)
        if not np.all(np.isfinite(noise)):
            self.logger.warning(f"Synthetic noise for {detector_name} contains NaN/Inf, replacing with Gaussian")
            noise = np.random.randn(len(noise)).astype(np.float32) * 1e-20
    ...
    def create_noise_augmentations(self, sample: Dict, k: int) -> List[Dict]:
        ...
        for aug_idx in range(1, k):
            ...
            for det_name in self.detectors:
                ...
                # Generate fresh noise (capture noise_type for metadata consistency)
                ...
                aug_sample["detector_data"][det_name] = {
                    ...
                    "noise": new_noise.astype(np.float32),
    ...
    def generate_dataset(
        ...
        def _track_sample(sample):
            ...
            This tracks the brightest/primary signal in the overlap, ensuring sample-level
            statistics reflect detection hierarchy. Overlapping signals are sorted by SNR
            before this call to ensure this behavior.
    ...
    def _sort_parameters_by_snr(self, parameters: List[Dict]) -> List[Dict]:
        """
        Sort parameters list by target_snr in descending order (brightest first).
        This ensures the first signal is always the highest-priority (primary) signal,
        which simplifies downstream analysis and SNR regime tracking.
        
        Args:
            parameters: List of parameter dicts
            
        Returns:
            Sorted list by target_snr (descending), or original list if sorting fails
        """
        if not parameters or not isinstance(parameters, list):
            return parameters
        
        try:
            # Extract SNR values with fallback to 15.0 if missing
            snr_values = []
            for p in parameters:
                if isinstance(p, dict):
                    snr = p.get("target_snr", 15.0)
                    snr_values.append(float(snr) if snr is not None else 15.0)
                else:
                    snr_values.append(15.0)
            
            # Sort by SNR descending (brightest first)
            sorted_pairs = sorted(zip(snr_values, parameters), key=lambda x: x[0], reverse=True)
            sorted_params = [p for _, p in sorted_pairs]
            
            return sorted_params
        except Exception as e:
            self.logger.debug(f"Failed to sort parameters by SNR: {e}, returning unsorted")
            return parameters
    ...
    def _estimate_snr_from_params(self, params: Dict) -> float:
        ...
        try:
            ...
            # ✅ Apply NSBH mass-based SNR boost if applicable (must match sample_nsbh_parameters)
            event_type = params.get('type') or params.get('event_type')
            if event_type == 'NSBH':
                bh_mass = max(m1, m2)  # Use max mass (BH is heavier in NSBH)
                if bh_mass < 10.0:
                    boost_mult = 1.0
                elif bh_mass < 30.0:
                    boost_mult = 1.25
                    ...
                    boost_mult = 2.0
                snr_estimate = snr_estimate * boost_mult
    ...
    def _get_actual_snr_regime(self, snr_value: float) -> str:
        """
        Determine which SNR regime an SNR value actually falls into.
        
        This is used to reclassify samples after SNR has been modified
        (e.g., NSBH boost applied), ensuring statistics track the true regime.
        
        Args:
            snr_value: Actual SNR value
            
        Returns:
            Regime name: 'weak', 'low', 'medium', 'high', 'loud'
        """
        snr_ranges = {
            'weak': (10.0, 15.0),
            'low': (15.0, 25.0),
        ...
        for regime, (min_snr, max_snr) in snr_ranges.items():
            if min_snr <= snr_value < max_snr:
                return regime
        
        # Out of range - assign to nearest regime
        if snr_value < 10.0:
            return 'weak'
        else:
            return 'loud'
    ...
    def _generate_single_sample(
        ...
        # ✅ CRITICAL FIX: For NSBH samples that drifted to higher SNR regimes due to boost,
        # pull them back to maintain correct regime distribution.
        # The parameter sampler applies pre-adjustment before boost, but may still exceed
        # regime bounds due to clipping or edge cases. This ensures statistics match targets.
        actual_regime = snr_regime
        if params is not None:
            actual_snr = params.get('network_snr', self._estimate_snr_from_params(params))
            actual_regime = self._get_actual_snr_regime(actual_snr)
            
            # If sample is in wrong regime, pull it back to sampled regime
            if snr_regime != actual_regime:
                snr_min, snr_max = self.parameter_sampler.snr_ranges[snr_regime]
                # Clamp SNR to sampled regime bounds
                # Use middle of the regime range to maximize variety within regime
                regime_mid = (snr_min + snr_max) / 2.0
                clamped_snr = np.clip(actual_snr, snr_min, snr_max - 0.01)
                # Prefer clamping to middle to avoid mode collapse at boundaries
                if actual_snr > snr_max:
                    clamped_snr = regime_mid
                
                params['target_snr'] = float(clamped_snr)
                params['network_snr'] = float(clamped_snr)
                actual_regime = snr_regime
        ...
        sample = {
            ...
            "metadata": {  # ← ADD THIS BLOCK
                ...
                "snr_regime": actual_regime,  # Store ACTUAL regime, not sampled regime
    ...
    def _generate_overlapping_sample(
        ...
        # CRITICAL: Sort parameters by target_snr (descending) to ensure first signal is brightest
        # This guarantees _track_sample counts the primary detection, not a secondary signal
        signal_params_list = self._sort_parameters_by_snr(signal_params_list)
        # Update target_snrs order to match sorted parameters (for consistency)
        target_snrs_dict = {params.get("geocent_time"): snr for params, snr in zip(signal_params_list, target_snrs)}
        target_snrs = [target_snrs_dict.get(params.get("geocent_time"), 15.0) for params in signal_params_list]
