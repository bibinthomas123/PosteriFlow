...
def sample_overlap_size(p_heavy=0.45, base_lambda=2.2, heavy_min=5, heavy_max=8):
    """
    Mixture sampler:
...
class GWDatasetGenerator:
    ...
    def generate_dataset(self,
        ...
        if self.extreme_enabled:
            ...
            self.logger.info(f"  → Pure regular (expected):   {n_regular_actual:>6,} (~{(n_regular_actual/n_samples)*100:.1f}%)")
            self.logger.info(f"  → Probabilistic extremes:    ~{n_expected_extremes:>5,} (~{(n_expected_extremes/n_samples)*100:.1f}%)")
            self.logger.info("")
            self.logger.info(f"Pre-merger samples:            {n_premerger:>6,} ({(n_premerger/n_samples)*100:.1f}%)")
            self.logger.info(f"Regular overlap samples:       {n_regular_overlap:>6,} ({(n_regular_overlap/n_samples)*100:.1f}%)")
            self.logger.info(f"Explicit edge cases:           {total_edge:>6,} ({(total_edge/n_samples)*100:.1f}%)")
            self.logger.info(f"{'─' * 80}")
                ...
                for i in range(n_regular_overlap):
                    ...
                    if quota_mode:
                        ...
                        for j in range(n_sigs):
                            snr_choice, evt_choice = _select_joint_cell()
                            forced_signals.append({'snr_regime': snr_choice, 'event_type': evt_choice})

                        # Bypass simulator to honor quotas
                        sample = self._generate_overlapping_sample(
                            sample_id, False, add_glitches, preprocess, forced_signals=forced_signals
                        )
                        ...
                        if self.simulation is not None:
                            try:
                                ...
                                self.logger.warning(f"Simulator overlap failed for {sample_id}: {e}")
                                ...
                                for params in sample.get('parameters', []):
                                    priority = self._estimate_snr_from_params(params)
                                    if 'target_snr' not in params:
                                        self._set_target_snr(params, priority, reason='overlap_fallback_assign')
                                    priorities.append(priority)
                                sample['priorities'] = priorities
                        else:
                            ...
                            for params in sample.get('parameters', []):
                                ...
                                if 'target_snr' not in params:
                                    self._set_target_snr(params, priority, reason='regular_overlap_legacy')
                                priorities.append(priority)
                            sample['priorities'] = priorities
        ...
        if np.random.random() < glitch_prob:
            ...
            for detector_name in sample['detector_data'].keys():
                ...
                if glitch_type == 'blip':
                    ...
                    glitch = glitch_amplitude * np.exp(-((np.arange(len(strain)) - glitch_time)**2) / (2 * glitch_width**2))
                    strain += glitch
                    ...
                    glitch = glitch_amplitude * np.sin(phase) * np.exp(-t / (len(strain) / (2 * self.sample_rate)))
                    strain += glitch
